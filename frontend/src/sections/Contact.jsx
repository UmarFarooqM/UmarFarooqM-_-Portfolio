...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const API_URL = `${import.meta.env.VITE_API_URL}/contact/add`;
      ...
      if (res.data?.success) {
        ...
        setIsError(true);
        setStatusMsg(`❌ ${res.data?.error || "Failed to send message."}`);
      }
    } catch (err) {
      ...
      if (err.code === "ECONNABORTED") {
        ...
        setStatusMsg("❌ API endpoint not found. Check backend route.");
      } else if (err.message.includes("Network Error")) {
        setStatusMsg("❌ Network error — check your internet connection.");
      } else {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
              <input
                ...
                required
                className="w-full px-3 sm:px-4 py-2 sm:py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white text-sm sm:text-base"
              />
              <input
                type="text"
                name="lastName"
            ...
            <textarea
              ...
              required
              rows="5"
              className="w-full px-3 sm:px-4 py-2 sm:py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white text-sm sm:text-base resize-none"
            />
